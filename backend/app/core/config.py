...
def normalize_database_url(value: str, *, app_env: str) -> str:
    ...
    PostgreSQL always requires encryption. Explicit local SQLite targets are
    supported for development and tests, but never for demo or production.
    """
    try:
        url = make_url(value)
    ...
    if backend == "sqlite":
        if app_env not in {"development", "local", "test"}:
...
class Settings(BaseSettings):
    ...
    def ensure_runtime_dirs(self) -> None:
        """Create non-database runtime directories and local SQLite parents."""
    ...
    def database_url(self, *, for_migration: bool = False) -> str:
        raw = (
            ...
            if for_migration and self.MIGRATION_DATABASE_URL
