...
async def run_cycle(case_id: str) -> None:
    ...
    async with factory() as db:
        ...
        if _ai_blocked(case, session, consent):
            return
        ...
        # End the read transaction before the slow path. The result is only a
        # candidate until authoritative state is reloaded during finalisation.
        await db.rollback()
...
def _ai_blocked(case: Case, session: Session, consent: str) -> bool:
    return (
        ...
        or session.human_joined
        or case.status == "taken_over"
        or runner.is_session_muted(session.id)
    )
...
async def _complete_cycle(
    ...
    Slow assessment happens before this function. The stable per-session lock
    orders the local worker's commit/publication against takeover, while fresh
    database reads provide the authoritative consent and takeover decision.
    """
    events: List[tuple] = []
    factory = session_factory()
    async with runner.session_lock(session_id):
        if runner.is_session_muted(session_id):
            return
        ...
        async with factory() as db:
            ...
            # This is the persistence boundary: authoritative state was just
            # reloaded while takeover is excluded by the session lock.
            try:
            ...
            for a in result["alerts"]:
                ...
                if prior is None:
                    ...
                    events.append(("alert.safety", _alert_event(
                        a["type"], a["severity"], a["evidence_turn_ids"])))
                    continue
                prior.evidence_turn_ids = sorted(
                    set(prior.evidence_turn_ids or []) | set(a["evidence_turn_ids"]))
                if SEVERITY_RANK.get(a["severity"], 0) > SEVERITY_RANK.get(prior.severity, 0):
                    ...
                    events.append(("alert.safety", _alert_event(
                        a["type"], a["severity"], prior.evidence_turn_ids)))
            ...
            # Recheck immediately before publication. The coordination lock
            # prevents a local takeover between this check and the publishes.
        ...
        # Publish after commit and after closing the DB transaction, while the
        # same session coordination lock still orders this before takeover.
        ...
        hub.publish(session_id, "escalation.packet", {
            "case_id": case_id, "band": result["band"], "alerts": alerts_now,
