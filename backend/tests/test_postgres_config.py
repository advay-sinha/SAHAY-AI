...
class TestDatabaseUrlConfiguration(unittest.TestCase):
    ...
    def test_sqlite_is_available_locally_but_not_in_demo_or_production(self):
        for app_env in ("development", "local", "test"):
            self.assertEqual(
                normalize_database_url("sqlite:///:memory:", app_env=app_env),
                "sqlite+aiosqlite:///:memory:",
            )
        for app_env in ("demo", "production"):
            with self.assertRaises(DatabaseConfigurationError):
                normalize_database_url("sqlite+aiosqlite:///:memory:", app_env=app_env)
...
class TestPostgresqlMetadata(unittest.TestCase):
    ...
    def test_security_migration_names_exactly_the_model_tables(self):
        ...
        merge_path = (
            ...
            / "alembic"
            / "versions"
            / "e4b7f8a9c012_merge_postgresql_and_pc11.py"
        )
        merge_spec = importlib.util.spec_from_file_location("merged_security_revision", merge_path)
        ...
        secured_tables = set(module.APPLICATION_TABLES) | set(merge_module.APPLICATION_TABLES)
        self.assertEqual(secured_tables, set(Base.metadata.tables))
