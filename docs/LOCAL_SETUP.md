...
- **Proposed contract change (not implemented):** move the socket token out of the URL — for example into `Sec-WebSocket-Protocol`, or a first `auth` message before any other frame. This changes CONTRACTS.md §1 and needs a `type:contract` issue with all three leads approving.
