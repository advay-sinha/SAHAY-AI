...
function createSessionStore({ apiUrl, fetchImpl, timeoutMs, onCredentialChange }) {
  ...
  function publishCredential(value) {
    if (typeof onCredentialChange !== 'function') return;
    try {
      onCredentialChange(value);
    } catch {
      // The private transport bridge cannot interrupt store state changes.
    }
  }
  ...
  async function attemptCreation(consent, lang, attempt) {
    ...
    publishCredential(result.session);
