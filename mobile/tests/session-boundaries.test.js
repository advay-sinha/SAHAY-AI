 ...
 *
 * The combined slice owns REST session creation, one victim-safe timeline,
 * and the existing authenticated live-text socket. The offline queue, storage,
...
test("only the combined provider imports the socket and no module imports the offline queue", () => {
  const offences = [];
  for (const { file, code } of allProductionSources()) {
    if (file === path.join("src", "net", "socket.ts")) continue;
    for (const spec of importsOf(code)) {
      if (/(?:^|\/)queue$/.test(spec)) offences.push(`${file} -> ${spec}`);
      if (/(?:^|\/)socket$/.test(spec)
          && file !== path.join("src", "session", "SessionProvider.tsx")) {
        offences.push(`${file} -> ${spec}`);
      }
    }
  }
  assert.deepEqual(offences, []);
  assert.match(read("src", "session", "SessionProvider.tsx"), /from ["']\.\.\/net\/socket["']/);
});

test("SessionSocket remains the only module that constructs a WebSocket", () => {
...
test("chat and handoff use the combined session while talk and home stay transport-free", () => {
  ...
  assert.match(chat, /const \{ session, events, sendChat \} = useSession\(\)/);
  assert.match(chat, /onSend=\{sendChat\}/);
  assert.match(chat, /receivedMessages=\{events\}/);
  assert.match(handoff, /const \{ requestHuman \} = useSession\(\)/);
  assert.match(handoff, /onRequestHuman=\{requestHuman\}/);
});
